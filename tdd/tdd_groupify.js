...
async function summerize_this_is_last_step_before_db_insert_test() {
  ...
  let keys = Object.keys(day_group_entries_actual)
  return day_group_entries_actual[keys[0]]


}
async function db_create_table_test(oneThingToRepresentATable) { 
  console.log( "TOOO WRITE / CREATE / POPULATE table ")
  console.log( oneThingToRepresentATable)
...
async function main() {
  ...
  const oneThingToRepresentATable = await summerize_this_is_last_step_before_db_insert_test()
  await db_create_table_test(oneThingToRepresentATable)
